...
答：根據部署場景公開程度，可以分為公有鏈（Public Chain）、聯盟鏈（Consortium Chain）和私有鏈（Private Chain）；從功能上看，可以分為以支持數字貨幣為主的數字貨幣區塊鏈（如比特幣網絡）、支持智能合約的通用區塊鏈（如以太坊網絡）、面向複雜商業應用場景支持權限管理的分佈式帳本平臺（如超級帳本）。
